 ...
 * Tablero de reportes.
 *
 * Una sola pantalla para los dos roles: el admin ve el negocio entero,
 * el supervisor solo lo que cargo el. Lo que cambia no es que se
 * escondan tarjetas, sino el alcance con el que se instancia el
 * service: para un supervisor, las consultas de recaudacion global
 * directamente no se corren.
 */
...
<main class="pantalla">
    ...
    <?php else: ?>
        ...
        <span class="rotulo d-block mb-2">Tu trabajo<?= $filtro->hayAlguno() ? ' en el período filtrado' : '' ?></span>

        <div class="row g-2 mb-4">
            <div class="col-6">
                <div class="tarjeta-dato">
                    <div class="tarjeta-dato__valor"><?= (int) $resumen['jugadas'] ?></div>
                    <div class="tarjeta-dato__rotulo">Jugadas que cargaste</div>
                    <div class="tarjeta-dato__pie"><?= e(formatPesos($resumen['recaudado'])) ?> cobrados</div>
                </div>
            </div>
            <div class="col-6">
                <div class="tarjeta-dato">
                    <div class="tarjeta-dato__valor"><?= (int) $resumen['clientes'] ?></div>
                    <div class="tarjeta-dato__rotulo">Clientes que te jugaron</div>
                </div>
            </div>
            <div class="col-6">
                <div class="tarjeta-dato">
                    <div class="tarjeta-dato__valor"><?= $clientes ?></div>
                    <div class="tarjeta-dato__rotulo">Clientes que diste de alta</div>
                </div>
            </div>
            <div class="col-6">
                <div class="tarjeta-dato">
                    <div class="tarjeta-dato__valor"><?= $sorteos ?></div>
                    <div class="tarjeta-dato__rotulo">Sorteos que cargaste</div>
                </div>
            </div>
        </div>
    ...
    <div class="d-flex align-items-baseline justify-content-between gap-2 mb-2">
        <span class="rotulo">
            <?= $alcance->esAdmin() ? 'Recaudación por semana' : 'Lo que cargaste por semana' ?>
        </span>
        <?php if ($tope > 0): ?>
                   ...
                   href="<?= APP_URL ?>/admin/reportes/jugadas.php?ciclo=<?= (int) $c['id'] ?>">
                    ...
                    <div class="d-flex justify-content-between align-items-center gap-2 mt-2">
                        <span class="fila__meta">
                            ...
                            <?php if ($alcance->esAdmin() && (float) $c['monto_arrastrado'] > 0): ?>
