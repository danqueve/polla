...
- **Ranking del sábado**: `admin/sabados/ver.php` y la pantalla nueva `portal/ranking_sabado.php` muestran quién va con más aciertos acumulados contra los turnos ya cargados del sábado en curso (unión de los turnos, no "mejor turno individual" — esa es la semántica de `PortalService::evaluar()`, que decide ganadores; esta es solo un indicador informal de progreso). Lo puede ver cualquier cliente logueado, no solo quien jugó ese sábado, pero solo se expone nombre y N° de cliente — nunca DNI, teléfono ni los números jugados por otro. Un cliente con más de una jugada aparece una sola vez, con su mejor resultado. Cálculo centralizado en `PortalService::numerosSalidos()`/`ranking()`, reemplazando el cálculo que antes estaba duplicado inline en `admin/sabados/ver.php` y `admin/ciclos/ver.php`.
