<?php
/**
 * Ranking de aciertos de la semana en curso: mismo criterio que
 * ranking_sabado.php (cualquier cliente logueado lo puede ver, no solo
 * quien jugo -- PortalService::ranking() recibe las jugadas de TODOS
 * los clientes del ciclo a proposito). Solo se muestra nombre y N de
 * cliente, nunca DNI, telefono ni los numeros jugados por otro.
 */
...
$ciclo   = (new CicloService($db))->obtenerCicloActivo(CicloService::TIPO_SEMANAL);
...
$pageTitle  = 'Ranking de la semana · ' . APP_NAME;
...
<main class="pantalla">
    ...
    <a href="<?= APP_URL ?>/portal/index.php?tipo=<?= CicloService::TIPO_SEMANAL ?>"
       class="btn btn-sm btn-outline-secondary mb-3">
        <i class="bi bi-arrow-left"></i> Mis jugadas
    </a>

    <h1 class="pantalla__titulo">Ranking de la semana</h1>
    <p class="pantalla__bajada">
        Quién va anotando más aciertos contra los sorteos ya salidos —
        <?= count($lista) ?>/5 sorteos cargados. Se actualiza solo a medida
        que Decena de Oro va cargando cada sorteo.
    </p>
    ...
    <?php if (!$lista): ?>
        <div class="vacio tarjeta">
            ...
            <p class="fila__meta mb-0">
                El ranking aparece apenas Decena de Oro cargue el primer sorteo de la semana.
            </p>
        </div>
    <?php elseif (!$ranking): ?>
        <div class="vacio tarjeta">
            <i class="bi bi-ticket-perforated" aria-hidden="true"></i>
            Todavía no hay jugadas cargadas esta semana.
        </div>
    <?php else: ?>
